...
class UFUnsatCoreTest : public ::testing::Test, public UnsatCoreTestBase<Logic> {
protected:
    UFUnsatCoreTest() : UnsatCoreTestBase(Logic_t::QF_UF) {}
    void SetUp() override {
...
TEST_F(UFUnsatCoreTest, Bool_Simple) {
    ...
    EXPECT_TRUE(coreTerms[0] == b1 or coreTerms[1] == b1);
    EXPECT_TRUE(coreTerms[0] == nb1 or coreTerms[1] == nb1);
}
...
TEST_F(UFUnsatCoreTest, Bool_ReuseProofChain) {
    ...
    auto & coreTerms = core->getTerms();
    ASSERT_EQ(coreTerms.size(), 2);
    EXPECT_TRUE(coreTerms[0] == a1 or coreTerms[1] == a1);
    EXPECT_TRUE(coreTerms[0] == a2 or coreTerms[1] == a2);
}
...
TEST_F(UFUnsatCoreTest, UF_Simple) {
    ...
    ASSERT_EQ(coreTerms.size(), 2);
    EXPECT_TRUE(coreTerms[0] == a1 or coreTerms[1] == a1);
    EXPECT_TRUE(coreTerms[0] == a3 or coreTerms[1] == a3);
}

/// Pure arrays
class AXUnsatCoreTest : public ::testing::Test, public UnsatCoreTestBase<Logic> {
protected:
    AXUnsatCoreTest() : UnsatCoreTestBase(Logic_t::QF_AX) {}
    void SetUp() override {
...
TEST_F(AXUnsatCoreTest, AX_Simple) {
    ...
    auto & coreTerms = core->getTerms();
    ASSERT_EQ(coreTerms.size(), 2);
    auto isInCore = [&](PTRef fla) -> bool { return std::find(coreTerms.begin(), coreTerms.end(), fla) != coreTerms.end(); };
    EXPECT_TRUE(isInCore(a1));
    EXPECT_TRUE(isInCore(a2));
}

/// Linear integer arithmetic
class LIAUnsatCoreTest : public ::testing::Test, public UnsatCoreTestBase<ArithLogic> {
protected:
    LIAUnsatCoreTest() : UnsatCoreTestBase(Logic_t::QF_LIA) {}
    void SetUp() override {
        x = logic.mkIntVar("x");
        y = logic.mkIntVar("y");
        z = logic.mkIntVar("z");
...
class ALIAUnsatCoreTest : public ::testing::Test, public UnsatCoreTestBase<ArithLogic> {
protected:
    ALIAUnsatCoreTest() : UnsatCoreTestBase(Logic_t::QF_ALIA) {}
    void SetUp() override {
