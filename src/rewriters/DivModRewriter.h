...
namespace opensmt {
        ...
        PTRef rewrite(PTRef term) override {
            ...
            if (logic.isIntDiv(symRef) || logic.isMod(symRef)) {
                ...
                if (not inCache) {
                    ...
                    // 0 <= modVar <= |dividend| - 1
                    definitions.push(
                        logic.mkAnd(logic.mkEq(dividend, logic.mkPlus(logic.mkTimes(divisor, divVar), modVar)),
    ...
    public:
        explicit DivModRewriter(ArithLogic & logic)
            : Rewriter<DivModConfig>(logic, config),
              logic(logic),
              config(logic) {}
