...
std::unique_ptr<SMTSolver> MainSplitter::createInnerSolver(SMTConfig & config, THandler & thandler, PTPLib::net::Channel<PTPLib::net::SMTS_Event, PTPLib::net::Lemma> & ch) {
...
std::vector<std::string> MainSplitter::getPartitionClauses() const {
    ...
    assert(verifyPartitions(partitionsTr));
