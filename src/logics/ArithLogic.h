...
namespace opensmt {
    ...
    public:
        ...
        PTRef mkIntConst(Number const & c) {
            if (not hasIntegers()) { throw OsmtApiException("Create Int constant in non-integral logic"); }
            return mkConst(getSort_int(), c);
        }
        PTRef mkRealConst(Number const & c) {
            if (not hasReals()) { throw OsmtApiException("Create Real constant in non-real logic"); }
            return mkConst(getSort_real(), c);
        }
        PTRef mkIntVar(char const * name) {
            if (not hasIntegers()) { throw OsmtApiException("Create Int var in non-integral logic"); }
            return mkVar(sort_INT, name, false);
        }
        PTRef mkRealVar(char const * name) {
            if (not hasReals()) { throw OsmtApiException("Create Real var in non-real logic"); }
        ...
        void checkHasReals() const {
            if (not hasReals()) throw OsmtApiException("Reals not defined for logic");
        }
        void checkHasIntegers() const {
            if (not hasIntegers()) throw OsmtApiException("Integers not defined for logic");
        }
        ...
        void checkSortInt(PTRef tr) {
            if (getSortRef(tr) != getSort_int()) throw OsmtApiException("Expected integral sort");
        }
        void checkSortReal(PTRef tr) {
            if (getSortRef(tr) != getSort_real()) throw OsmtApiException("Expected real sort");
        }
