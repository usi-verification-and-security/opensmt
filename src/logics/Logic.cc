...
#include <common/OsmtApiException.h>
#include <common/OsmtInternalException.h>
...
namespace opensmt {
    ...
    PTRef Logic::resolveTerm(char const * s, vec<PTRef> && args, SRef sortRef, SymbolMatcher symbolMatcher) {
        ...
        if (sref == SymRef_Undef) {
            ...
            throw OsmtApiException("Unknown symbol `" + std::string(s) + ' ' + argSortsString +
                                   (sortRef != SRef_Undef ? "/ " + printSort(sortRef) : "") + "'");
        }
        assert(sref != SymRef_Undef);
        PTRef rval = insertTerm(sref, std::move(args));
        if (rval == PTRef_Undef) throw OsmtApiException("Error in resolveTerm\n");
    ...
    PTRef Logic::mkIte(vec<PTRef> && args) {
        ...
        if (sr != getSortRef(args[2])) { throw OsmtApiException("ITE arguments need to have same return sorts"); }
...
#ifndef NDEBUG
        std::string why;
        if (not typeCheck(sym, terms, why)) { throw OsmtInternalException(why); }
#endif
    ...
    bool Logic::getNewFacts(PTRef root, MapWithKeys<PTRef, lbool, PTRefHash> & facts) {
        ...
        for (int i = 0; i < facts_dbg.size(); i++)
            cerr << (facts_dbg[i].data == l_True ? "" : "not ") << printTerm(facts_dbg[i].key) << " ("
                 << facts_dbg[i].key.x << ")" << endl;
#endif
    }
    ...
    PTRef Logic::learnEqTransitivity(PTRef formula) {
        ...
        while (queue.size() != 0) {
            ...
            bool const cond1 = isOr(tr) && t.size() == 2 && isAnd(t[0]) && isAnd(t[1]) &&
                               isEquality(getPterm(t[0])[0]) && isEquality(getPterm(t[0])[1]) &&
                               isEquality(getPterm(t[1])[0]) && isEquality(getPterm(t[1])[1]);
    ...
    PTRef Logic::instantiateFunctionTemplate(TemplateFunction const & tmplt, vec<PTRef> const & args) {
        ...
        if (args.size() != targs.size()) { throw OsmtApiException("Function instantiation argument count mismatch"); }
    ...
    bool Logic::typeCheck(SymRef sym, vec<PTRef> const & args, std::string & why) const {
        ...
        if (symbol.chainable() or symbol.pairwise()) {
            ...
            auto allButFirstSortEqualSecond = [&](vec<PTRef> const & args) {
                return std::all_of(args.begin() + 1, args.end(),
