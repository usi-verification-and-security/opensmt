...
namespace opensmt {
        ...
        Logic::SubstMap collectConstantSubstitutions(ArithLogic & logic, std::vector<poly_t> & zeroPolynomials) {
            ...
            while (true) {
                ...
                for (std::size_t i = 0; i < zeroPolynomials.size(); ++i) {
                    ...
                    if (poly.size() == 2 and
                        (poly.begin() + 1)->var == PTRef_Undef) { // poly is "a*x + c = 0" for c != 0
    ...
    void SimplifyConstSum::constSimplify(SymRef s, vec<PTRef> const & terms, SymRef & s_new,
    ...
    void SimplifyConstDiv::constSimplify(SymRef s, vec<PTRef> const & terms, SymRef & s_new,
                                         vec<PTRef> & terms_new) const {
        assert(terms_new.size() == 0);
        assert(terms.size() <= 2);
    ...
    std::string ArithLogic::printTerm_(PTRef tr, bool ext, bool safe) const {
        if (isNumConst(tr)) {
            ...
            if (is_div) {
                ...
                int written =
                    is_neg ? asprintf(&tmp, "(/ (- %s) %s)", nom, den) : asprintf(&tmp, "(/ %s %s)", nom, den);
                assert(written >= 0);
                (void)written;
                str << tmp;
                if (ext) { str << " <" << tr.x << ">"; }
    ...
    opensmt::pair<Number, PTRef> ArithLogic::sumToNormalizedRealPair(PTRef sum) {
        ...
        PTRef normalizedSum =
            varFactors.size() == 1 ? varFactors[0] : mkFun(get_sym_Real_PLUS(), std::move(varFactors));
