...
TRes LASolver::cutFromProof() {
    ...
    std::vector<DefiningConstraint> singleVarConstraints;
    for (LVRef var : int_vars) {
        ...
        if (isOnLower and isOnUpper) {
            ...
            if (logic.isVar(term)) {
                singleVarConstraints.push_back(DefiningConstraint{term, rhs});
            } else {
...
//        std::cout << logic.pp(term) << " = " << rhs << std::endl;
    ...
    constraints.insert(constraints.begin(), equalities.begin(), equalities.end());
    constraints.insert(constraints.end(), singleVarConstraints.begin(), singleVarConstraints.end());
