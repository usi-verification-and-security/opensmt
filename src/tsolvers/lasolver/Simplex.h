...
public:
    ...
    inline bool isModelStrictlyUnderUpperBound(LVRef v) const {
        return (!model->hasUBound(v) || model->read(v) < model->Ub(v));
    }
    // No lower bound count as -infinity
    inline bool isModelStrictlyOverLowerBound(LVRef v) const {
        return (!model->hasLBound(v) || model->read(v) > model->Lb(v));
    }

    void newNonbasicVar(LVRef v) {
        newVar(v);
        tableau.newNonbasicVar(v);
    }
    void nonbasicVar(LVRef v) {
        newVar(v);
        tableau.nonbasicVar(v);
    }
    void newRow(LVRef x, std::unique_ptr<Tableau::Polynomial> poly) {
        newVar(x);
        tableau.newRow(x, std::move(poly));
    }
    Explanation getConflictingBounds(LVRef x, bool conflictOnLower);
    bool checkValueConsistency() const;
    bool invariantHolds() const;

    Real computeDelta() const;
    Delta getValuation(LVRef) const; // Understands also variables deleted by gaussian elimination
    //    Delta read(LVRef v) const { assert(!tableau.isQuasiBasic(v)); return model->read(v); } // ignores unsafely
    //    variables deleted by gaussian elimination
    LABoundRef const readLBoundRef(LVRef const & v) const { return model->readLBoundRef(v); }
    LABoundRef const readUBoundRef(LVRef const & v) const { return model->readUBoundRef(v); }
    Delta const & Lb(LVRef v) const { return model->Lb(v); }
    Delta const & Ub(LVRef v) const { return model->Ub(v); }
    ...
    void boundActivated(LVRef v) {
        ...
        if (tableau.isQuasiBasic(v)) { quasiToBasic(v); }
        ++boundsActivated[getVarId(v)];
    }
    void boundDeactivated(LVRef v) {
        assert(boundsActivated[getVarId(v)] > 0);
        --boundsActivated[getVarId(v)];
        if (getNumOfBoundsActive(v) == 0 && tableau.isBasic(v)) { tableau.basicToQuasi(v); }
    }

    lbool getPolaritySuggestion(LVRef var, LABoundRef pos, LABoundRef neg) const {
        if (tableau.isQuasiBasic(var)) { (const_cast<Simplex *>(this))->quasiToBasic(var); }
        ...
        // of variable "x" as <0,-1/2> with term "x >= 0". The positive bound is lower with value <0,0> and the
        // negative bound is upper with value <0, -1>. Then both "positive" and "negative" will be false
...
private:
    ...
    void changeValueBy(LVRef, Delta const &); // Updates the bounds after constraint pushing
    ...
    Delta const overBound(LVRef) const;

    // Model & bounds
    LABoundRef const getBound(LVRef v, int idx) const; //{ return boundStore.getBoundByIdx(v, idx); }
    bool isUnbounded(LVRef v) const;

    bool valueConsistent(LVRef v) const; // Debug: Checks that the value of v in the model is consistent with the
                                         // evaluated value of the polynomial of v in the same model.
