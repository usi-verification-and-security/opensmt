...
template<class T> VertexRef STPStore<T>::createVertex() {
...
template<class T> void STPStore<T>::setNegation(EdgeRef a, EdgeRef b) {
    edges[a.x].neg = b;
    edges[b.x].neg = a;
}

template<class T> void STPStore<T>::clear() {
    edges.clear();
