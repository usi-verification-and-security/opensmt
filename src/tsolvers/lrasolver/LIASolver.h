...
class LIASolverStats: public TSolverStats  //PS. check all opensmt::real and should I change them to opensmt::integer
{
public:
    //int num_pivot_ops;
    //int num_bland_ops;
    ...
    void printStatistics(ostream& os)
    {
        ...
        os << "; Simplex time.............: " << bb_timer.getTime() << " s\n";
    }
};
...
//    vector<opensmt::Real*> numbers_pool;    // Collect numbers.  Should work as a simple memory managemet system
    ...
    opensmt::Real delta; // The size of one delta.  Set through computeModel() //PS. shall I declare type of delta integer?
    unsigned bland_threshold;
    LIASolverStats tsolver_stats;
...
//    void addVarToRow( LVRef, LVRef, opensmt::Real*);
    ...
    vec<opensmt::Real*> concrete_model;    // Save here the concrete model for the vars indexed by Id //PS. concrete model real or integer?
