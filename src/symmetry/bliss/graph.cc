...
Digraph<T>::Vertex::add_edge_to(const unsigned int other_vertex)
{
...
template<typename T> void
Digraph<T>::Vertex::add_edge_from(const unsigned int other_vertex)
{
...
template<typename T>void
Digraph<T>::Vertex::remove_duplicate_edges(std::vector<bool>& tmp)
{
#if defined(BLISS_CONSISTENCY_CHECKS)
  /* Pre-conditions  */
...
template<typename T>
Digraph<T>::Digraph(const unsigned int nof_vertices)
{
...
Digraph<T>::get_vertex_label(const unsigned int vertex) const
{
  return vertices[vertex].label;
}

template<typename T> const T*
Digraph<T>::get_vertex_value(const unsigned int vertex) const
{
  return vertices[vertex].value;
}

template<typename T> unsigned int
Digraph<T>::add_vertex(const unsigned int color, const std::string label, const T* value)
{
  const unsigned int new_vertex_num = vertices.size();
  vertices.resize(new_vertex_num + 1);
  vertices.back().color = color;
  vertices.back().label = label;
  vertices.back().value = value;
  return new_vertex_num;
}


template<typename T> void
Digraph<T>::add_edge(const unsigned int vertex1, const unsigned int vertex2)
{
...
template<typename T> void
Digraph<T>::change_color(const unsigned int vertex, const unsigned int new_color)
{
...
template<typename T> Digraph<T>*
Digraph<T>::permute(const std::vector<unsigned int>& perm) const
{
  Digraph<T>* const g = new Digraph<T>(get_nof_vertices());
  for(unsigned int i = 0; i < get_nof_vertices(); i++)
    {
...
template<typename T> Digraph<T>*
Digraph<T>::permute(const unsigned int* const perm) const
{
  Digraph<T>* const g = new Digraph<T>(get_nof_vertices());
  for(unsigned int i = 0; i < get_nof_vertices(); i++)
    {
...
template<typename T> void
Digraph<T>::write_dot(const char* const filename)
{
...
template<typename T> void
Digraph<T>::write_dot(FILE* const fp)
{
  remove_duplicate_edges();
  ...
  unsigned int vnum = 0;
  for(typename std::vector<Vertex>::const_iterator vi = vertices.begin();
      vi != vertices.end();
      vi++, vnum++)
    {
      const Vertex& v = *vi;
      //fprintf(fp, "v%u [label=\"%u:%u (%s)\"];\n", vnum, vnum, v.color, v.label.c_str());
      fprintf(fp, "v%u [label=\"(%u) %u, %s\"];\n", vnum, vnum, v.color, v.label.c_str());
      for(std::vector<unsigned int>::const_iterator ei = v.edges_out.begin();
	  ei != v.edges_out.end();
	  ei++)
...
template<typename T> void
Digraph<T>::remove_duplicate_edges()
{
  std::vector<bool> tmp(get_nof_vertices(), false);

  for(typename std::vector<Vertex>::iterator vi = vertices.begin();
      vi != vertices.end();
      vi++)
    {
...
template<typename T> Digraph<T>*
Digraph<T>::read_dimacs(FILE* const fp, FILE* const errstr)
{
  Digraph<T>* g = 0;
...
template<typename T> void
Digraph<T>::write_dimacs(FILE* const fp)
{
  remove_duplicate_edges();
  sort_edges();
...
template<typename T> unsigned int
Digraph<T>::vertex_color_invariant(const Digraph<T>* const g, const unsigned int vnum)
{
  return g->vertices[vnum].color;
}

template<typename T> unsigned int
Digraph<T>::indegree_invariant(const Digraph<T>* const g, const unsigned int vnum)
{
  return g->vertices[vnum].nof_edges_in();
}

template<typename T> unsigned int
Digraph<T>::outdegree_invariant(const Digraph<T>* const g, const unsigned int vnum)
{
  return g->vertices[vnum].nof_edges_out();
}

template<typename T> unsigned int
Digraph<T>::selfloop_invariant(const Digraph<T>* const g, const unsigned int vnum)
{
...
Digraph<T>::refine_according_to_invariant(unsigned int (*inv)(const Digraph<T>* const g,
...
template<typename T> bool
Digraph<T>::split_neighbourhood_of_cell(Partition::Cell* const cell)
{
...
template<typename T> bool
Digraph<T>::split_neighbourhood_of_unit_cell(Partition::Cell* const unit_cell)
{
...
template<typename T> void
Digraph<T>::make_initial_equitable_partition()
{
...
template<typename T> Partition::Cell*
Digraph<T>::find_next_cell_to_be_splitted(Partition::Cell* cell)
{
  switch(sh) {
 ...
 */
template<typename T> Partition::Cell*
Digraph<T>::sh_first_max_neighbours()
{
  Partition::Cell* best_cell = 0;
  int best_value = -1;
 ...
 */
template<typename T> Partition::Cell*
Digraph<T>::sh_first_smallest_max_neighbours()
{
  Partition::Cell* best_cell = 0;
  int best_value = -1;
  unsigned int best_size = UINT_MAX;
 ...
 */
template<typename T> Partition::Cell*
Digraph<T>::sh_first_largest_max_neighbours()
{
  Partition::Cell* best_cell = 0;
  int best_value = -1;
  unsigned int best_size = 0;
...
template<typename T> void
Digraph<T>::initialize_certificate()
{
  certificate_index = 0;
 ...
 */
template<typename T> bool
Digraph<T>::is_automorphism(unsigned int* const perm)
{
...
template<typename T> bool
Digraph<T>::is_automorphism(const std::vector<unsigned int>& perm) const
{
...
template<typename T> bool
Digraph<T>::nucr_find_first_component(const unsigned int level)
{
...
unsigned int
Graph::add_vertex(const unsigned int color, const std::string label)
{
...
std::string
Graph::get_vertex_label(const unsigned int vertex) const
{
  return vertices[vertex].label;
}
