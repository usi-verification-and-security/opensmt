...
//    int derivations = 0;
    for (Var v(idx % nVars()); !score->isAlreadyChecked(v) && (close_to_prop!=0 || bound_prop!=0); v = Var((idx + (++i)) % nVars())) {
...
//              printf("%d\n", i);
  ...
  #endif
//                      printf("Variable: %d, Round: %d\n", v, score->getLatestRound());
//                      derivations++;
//                      if(derivations>close_to_prop/10){
//                        derivations=0;
                      score->updateRound();
//                      }
                      break;
