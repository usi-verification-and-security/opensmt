    ...
    while (next == var_Undef || value(next) != l_Undef || !decision[next]) {
        if (order_heap.empty()) {
            ...
            if (splitConfig.split_on && next != var_Undef) {
                if (splitConfig.split_preference == sppref_tterm && !theory_handler.isTheoryTerm(next)) {
                    discarded.push(next);
                    next = var_Undef;
                }
                else if (splitConfig.split_preference == sppref_bterm && theory_handler.isTheoryTerm(next)) {
                    discarded.push(next);
                    next = var_Undef;
                }
            }
