...
void ProofGraph::produceSingleInterpolant( vector<PTRef>& interpolants, const ipartitions_t& A_mask)
{
	...
	checkInterAlgo();
    /*
	if( config.itp_bool_alg() == 3 )
	{
#ifndef FULL_LABELING
		opensmt_warning("Please compile with --enable-fulllabeling to enable minimal interpolation");
		return;
#endif
	}
    */
	...
	if ( enabledInterpVerif() ) verifyInterpolant(getRoot()->getPartialInterpolant(), A_mask );
...
PTRef ProofGraph::compInterpLabelingInnerSimple( ProofNode * n, const ipartitions_t & A_mask )
{
	...
	assert( partial_interp_ant1 != PTRef_Undef ); assert( partial_interp_ant2!= PTRef_Undef );
