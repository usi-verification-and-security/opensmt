...
namespace opensmt {
    ...
    public:
        ...
        SingleInterpolationComputationContext(SMTConfig const & config, Theory & theory, TermMapper & termMapper,
                                              PartitionManager & pmanager, ProofGraph const & proof,
                                              ipartitions_t const & A_mask);

        inline bool isColoredA(ProofNode const & n, Var v) const {
            return nodeData[n.getId()].isColoredA(getSharedVarIndex(v));
        }

        inline bool isColoredB(ProofNode const & n, Var v) const {
            return nodeData[n.getId()].isColoredB(getSharedVarIndex(v));
        }
        ...
        inline void setPartialInterpolant(ProofNode const & n, PTRef itp) {
            nodeData[n.getId()].partialInterpolant = itp;
        }
        ...
        icolor_t getSharedVarColorInNode(Var v, ProofNode const & node) const {
            if (isColoredA(node, v))
                return icolor_t::I_A;
            else if (isColoredB(node, v))
                return icolor_t::I_B;
            else if (isColoredAB(node, v))
                return icolor_t::I_AB;

            throw OsmtInternalException("Variable " + std::to_string(v) + " has no color in clause " +
    ...
    namespace {
        ...
        icolor_t getClass(ipartitions_t const & mask, ipartitions_t const & A_mask) {
            ...
            else if (not in_A and in_B)
                clause_color = icolor_t::I_B;
            else if (in_A and in_B)
                clause_color = icolor_t::I_AB;
            else
                throw OsmtInternalException("No class could have been determined");
    ...
    std::unique_ptr<std::map<Var, icolor_t>> SingleInterpolationComputationContext::computePSFunction() const {
        ...
        for (auto leafId : proofGraph.getLeaves()) {
            ...
            if (n->getType() != clause_type::CLA_ORIG) { continue; }
            ...
            for (Lit l : n->getClause()) {
                ...
                if (col == icolor_t::I_A) {
                    ++occ_a[v];
                    if (occ_b.find(v) == occ_b.end()) { occ_b[v] = 0; }
                } else if (col == icolor_t::I_B) {
                    ++occ_b[v];
                    if (occ_a.find(v) == occ_a.end()) occ_a[v] = 0;
                }
            }
        }
    ...
    void SingleInterpolationComputationContext::checkInterAlgo() const {
        if (!(usingMcMillanInterpolation() || usingPudlakInterpolation() || usingMcMillanPrimeInterpolation() ||
              usingPSInterpolation() || usingPSWInterpolation() || usingPSSInterpolation()))
    ...
    void SingleInterpolationComputationContext::initTSolver() {
        ...
        for (auto id : leaves_ids) {
            ...
            auto const & clause = proofGraph.getNode(id)->getClause();
    ...
    bool SingleInterpolationComputationContext::verifyPartialInterpolantA(ProofNode const & n) {
        ...
        for (Lit l : cl) {
            ...
            if (var_color == icolor_t::I_A || var_color == icolor_t::I_AB) {
                ...
                if (sign(l)) { ptaux = logic.mkNot(ptaux); }
    ...
    SingleInterpolationComputationContext::SingleInterpolationComputationContext(
        SMTConfig const & config, Theory & theory, TermMapper & termMapper, PartitionManager & pmanager,
        ProofGraph const & proof, ipartitions_t const & A_mask)
        : logic(theory.getLogic()),
          config(config),
          pmanager(pmanager),
          proofGraph(proof),
          thandler(new THandler(theory, termMapper)),
          A_mask(A_mask) {
        ...
        for (Var v : vars) {
            ...
            if (v_class == icolor_t::I_A) {
                AB_vars_mapping[v] = -1;
            } else if (v_class == icolor_t::I_B) {
                AB_vars_mapping[v] = -2;
    ...
    PTRef SingleInterpolationComputationContext::produceSingleInterpolant() {
        ...
        for (size_t i = 0; i < proof_size; i++) {
            ...
            if (n->isLeaf()) {
                if (!isLeafClauseType(n->getType()))
                    throw OsmtInternalException("; Leaf node with non-leaf clause type");
    ...
    void SingleInterpolationComputationContext::labelLeaf(ProofNode & n, std::map<Var, icolor_t> const * PSFunction) {
        // Proof Sensitive
        if (usingPSInterpolation())
            setLeafPSLabeling(n, *PSFunction);
        else if (usingPSWInterpolation())
            setLeafPSWLabeling(n, *PSFunction);
        else if (usingPSSInterpolation())
            setLeafPSSLabeling(n, *PSFunction);
        // McMillan's system
        else if (usingMcMillanInterpolation())
            setLeafMcMillanLabeling(n);
        // Symmetric system
        else if (usingPudlakInterpolation())
            setLeafPudlakLabeling(n);
        // McMillan's prime system
        else if (usingMcMillanPrimeInterpolation())
            setLeafMcMillanPrimeLabeling(n);
        // Error
        else
            throw OsmtApiException("No interpolation algorithm chosen");
    }

    std::vector<Lit> SingleInterpolationComputationContext::getRestrictedNodeClause(ProofNode const & node,
                                                                                    icolor_t wantedVarClass) const {
    ...
    PTRef SingleInterpolationComputationContext::getInterpolantForOriginalClause(ProofNode const & node,
                                                                                 icolor_t clauseClass) const {
        if (clauseClass != icolor_t::I_A and clauseClass != icolor_t::I_B) {
            throw OsmtInternalException("Unexpected class");
        }
    ...
    PTRef SingleInterpolationComputationContext::computePartialInterpolantForTheoryClause(ProofNode const & n) {
        ...
        if (satisfiable) {
            ...
            throw OsmtInternalException(
                "Asserting negation of theory clause did not result in conflict in theory solver!");
        }
     ...
     * already present in the original formula (=> no mixed literals). Thus, we take the color of the clause as the
     * common color of both literals. If one literal is colored A and the other one B, then the split expression is
    ...
    PTRef SingleInterpolationComputationContext::compInterpLabelingInner(ProofNode & n) {
        ...
        // Alternative interpolant = ((NOT pivot) AND interpolant of ant1) OR (pivot AND interpolant of ant2)
            ...
            bool choose_alternative =
                usingAlternativeInterpolant()
                    ? decideOnAlternativeInterpolation(partial_interp_ant1, partial_interp_ant2, logic)
                    : false;
            ...
            throw OsmtInternalException("Pivot has no color");
    }
    ...
    void SingleInterpolationComputationContext::setLeafPSLabeling(ProofNode & n,
                                                                  std::map<Var, icolor_t> const & labels) {
    ...
    void SingleInterpolationComputationContext::setLeafPSWLabeling(ProofNode & n,
                                                                   std::map<Var, icolor_t> const & labels) {
        setLeafLabeling(n, [&](ProofNode & node, Var v) {
            auto it = labels.find(v);
            if (it->second == icolor_t::I_A)
    ...
    void SingleInterpolationComputationContext::setLeafPSSLabeling(ProofNode & n,
                                                                   std::map<Var, icolor_t> const & labels) {
        setLeafLabeling(n, [&](ProofNode & node, Var v) {
            auto it = labels.find(v);
            if (it->second == icolor_t::I_A)
    ...
    void SingleInterpolationComputationContext::setLeafPudlakLabeling(ProofNode & n) {
        setLeafLabeling(n, [&](ProofNode & node, Var v) { colorAB(node, v); });
    }

    void SingleInterpolationComputationContext::setLeafMcMillanLabeling(ProofNode & n) {
        setLeafLabeling(n, [&](ProofNode & node, Var v) { colorB(node, v); });
    }

    void SingleInterpolationComputationContext::setLeafMcMillanPrimeLabeling(ProofNode & n) {
        setLeafLabeling(n, [&](ProofNode & node, Var v) { colorA(node, v); });
    }

    /************************ INTERPOLATION CONTEXT ************************************************************/

    InterpolationContext::InterpolationContext(SMTConfig & c, Theory & th, TermMapper & termMapper,
                                               ResolutionProof const & proof, PartitionManager & pmanager)
        : config(c),
          theory(th),
        ...
        if (c.proof_reduce()) { reduceProofGraph(); }
    ...
    bool InterpolationContext::getPathInterpolants(vec<PTRef> & interpolants,
                                                   std::vector<ipartitions_t> const & A_masks) {
        assert(proof_graph);
        bool propertySatisfied = true;
        // check that masks are subset of each other
        assert(std::mismatch(A_masks.begin() + 1, A_masks.end(), A_masks.begin(),
                             [](auto const & next, auto const & previous) { return (previous & next) == previous; })
                   .first == A_masks.end());
    ...
    void InterpolationContext::transformProofForCNFInterpolants() {
        ...
        if (usingMcMillanInterpolation()) {
            proof_graph->transfProofForCNFInterpolants([](Var) {
    ...
    bool InterpolationContext::verifyInterpolant(PTRef itp, ipartitions_t const & A_mask) const {
    ...
    PTRef InterpolationContext::simplifyInterpolant(PTRef itp) const {
        bool const cannotSimplify = not logic.isBooleanOperator(itp) or logic.isNot(itp);
    ...
    void InterpolationContext::ensureNoLiteralsWithoutPartition() {
        ...
        if (!noPartitionVars.empty()) { proof_graph->eliminateNoPartitionTheoryVars(noPartitionVars); }
    }
} // namespace opensmt
