...
namespace {
    ...
    icolor_t getClass(ipartitions_t const & mask, ipartitions_t const & A_mask) {
        ...
        if (in_A and not in_B)
            ...
            throw InternalException("No class could have been determined");
...
bool SingleInterpolationComputationContext::verifyPartialInterpolant(ProofNode const & n) {
    ...
    if (!res) {
        assert(false);
        throw InternalException("Partial interpolant soundness does not hold for A");
    }
...
SingleInterpolationComputationContext::SingleInterpolationComputationContext(SMTConfig const & config, Theory & theory,
...
void SingleInterpolationComputationContext::labelLeaf(ProofNode & n, std::map<Var, icolor_t> const * PSFunction) {
    ...
    // Error
        ...
        throw ApiException("No interpolation algorithm chosen");
}
...
PTRef SingleInterpolationComputationContext::getInterpolantForOriginalClause(ProofNode const & node,
    ...
    if (clauseClass != icolor_t::I_A and clauseClass != icolor_t::I_B) { throw InternalException("Unexpected class"); }
...
PTRef SingleInterpolationComputationContext::computePartialInterpolantForSplitClause(ProofNode const & n) const {
    ...
    if (clauseColor != icolor_t::I_A and clauseColor != icolor_t::I_B) {
        assert(false);
        throw InternalException("Error in coloring split clause!");
    }
...
PTRef SingleInterpolationComputationContext::compInterpLabelingInner(ProofNode & n) {
    ...
    // Alternative interpolant = ((NOT pivot) AND interpolant of ant1) OR (pivot AND interpolant of ant2)
        ...
        bool choose_alternative = usingAlternativeInterpolant()
...
void SingleInterpolationComputationContext::setLeafLabeling(ProofNode & node, TFun colorABVar) {
    ...
    for (Lit l : cl) {
        ...
        if (var_class == icolor_t::I_AB) {
            ...
            InternalException("Variable has no class");
        }
    }
}
