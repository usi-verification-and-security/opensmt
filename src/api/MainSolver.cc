...
sstat MainSolver::simplifyFormulas() {
    ...
    for (std::size_t i = firstNotPreprocessedFrame; i < frames.frameCount(); ++i) {
        ...
        assert(frame.formulas.size() == 0 or frame.formulas.size() > preprocessedFrameAssertionsCount);
...
vec<PTRef> MainSolver::preprocessFormulasPerPartition(vec<PTRef> const & frameFormulas,
    ...
    assert(context.preprocessedFrameAssertionsCount >= 0);
    ...
    assert(processedFormulas.size() == formulasCountToProcess);
